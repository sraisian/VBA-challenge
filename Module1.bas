...
'Loop through each sheet in current active workbook
Set Workbook = ActiveWorkbook
For Each ws In Workbook.Worksheets
ws.Activate

    'Start looping at second row (ignore header row)
    datarow = 2
    ...
    For i = 2 To LastRow
        ...
        ElseIf ws.Cells(i, 1).Value <> ws.Cells(i + 1, 1).Value Then
            ...
            'Change color format to green if positive change
